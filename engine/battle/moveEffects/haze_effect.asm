...
; copy unmodified stats to battle stats	
	...
	ld a, [H_WHOSETURN]
	and a
	jr nz, .cureStatuses ;Flipped target of status cure.
...
; prevent the Pokemon from executing a move if it was asleep or frozen	
	ld a, $ff
	ld [de], a
;would fix freeze wakeup bug here, but pokemon with Haze can't unfreeze themselves (yet ;D )
...
; only cures statuses of the Pokemon not using Haze
	...
	and $ff ^((1 << UsingXAccuracy) | (1 << ProtectedByMist) | (1 << NeedsToRecharge) | (1 << GettingPumped) | (1 << Seeded)) 
