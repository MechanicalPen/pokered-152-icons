...
;ok, you can use a ball
	xor a
	ld [wd11c],a ;wCapturedMonSpecies
	ld a,[W_BATTLETYPE]
	cp a,2		;SafariBattle
...
.loop	;$56fa
	...
	cp a,POKE_BALL
	jr z,.checkForAilments ;any value is in range with pokeball.
	ld a,200
	cp b
...
.next8	;$5766
	ld [H_DIVISOR],a
	ld b,4
	call Divide	; W = ((MaxHP * 255) / BallFactor) / (CurHP / 4)
	ld a,[H_QUOTIENT + 2]
	and a
	jr z,.next9
	ld a,255 ; cap at 255.
...
.failedToCapture	;$578d
	...
	ld [H_MULTIPLICAND],a
	ld [H_MULTIPLICAND + 1],a
	ld a,[wEnemyMonCatchRate]	;enemy: Catch Rate (Use catchrate2 for Ditto??)
	ld [H_MULTIPLICAND + 2],a
	ld a,100
	ld [H_MULTIPLIER],a
...
; if it is active in battle
	...
	;res BadlyPoisoned,[hl] ; heal Toxic status (don't need to heal Toxic now that it is in status)
	;pop hl
	ld bc,30
	...
	predef DoubleOrHalveSelectedStats ;TODO: does this override burn/paralyze drops?
	jp .doneHealing
.healHP
...
.skipWakingUpEnemyParty
	...
	ld a,[hl]
	;player used an item so we don't need to prevent their move.
	and b ; remove Sleep status
	ld [hl],a
	ld hl,wEnemyMonStatus
	ld a,[hl]
	and a,SLP ; sleep mask
	jr z,.removeEnemySleepStatus
	; sleeping; need to prevent them from using a move they couldn't select.
	push hl
	ld hl, wEnemySelectedMove
	ld a, $ff
	ld [hl], a
	pop hl
.removeEnemySleepStatus
	and b ; remove Sleep status
	ld [hl],a
