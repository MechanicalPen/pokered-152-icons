...
; Reset player status on blackout.
	...
	ld [wFlags_0xcd60], a
	;;subtract money won. This isn't right.
	ld a, [wAmountMoneyWon] ;comment out to restore halving behavior
	ld [hMoneyInput], a
	ld a, [wAmountMoneyWon + 1] ;comment out to restore halving behavior
	ld [hMoneyInput + 1], a
	ld a, [wAmountMoneyWon + 2] ;comment out to restore halving behavior
	ld [hMoneyInput + 2], a
	call HasEnoughMoney
	...
	;ld [hMoneyInput + 2], a
	;xor a
	;ld [hMoneyTransform], a
	;ld [hMoneyTransform + 1], a
	;ld a, 2
	;ld [hMoneyTransform + 2], a
	;predef DivideBCDPredef3
	;ld a, [hMoneyTransform]
	;ld [wPlayerMoney], a
	;ld a, [hMoneyTransform + 1]
	;ld [wPlayerMoney + 1], a
	;ld a, [hMoneyTransform + 2]
	;ld [wPlayerMoney + 2], a
.lostmoney
	ld hl, wd732
	set 2, [hl]
...
SubtractAmountPaidFromMoney_: ; 6b21 (1:6b21)
	ld de,wPlayerMoney
	ld hl,hMoneyInput ; total price of items
...
.calcFreshStats
	...
	pop de
	ld a, [H_MULTIPLICAND+1] ;set max HP to max HP
	ld [de], a
	inc de
	ld a, [H_MULTIPLICAND+2]
	ld [de], a
	...
	call CalcStatsWithSkip	;calculate atk def spd spc
.done
	scf
	ret
...
; player coins
	...
	ld [hli], a
	ld [hl], a
; initialize variable sprite (see RedPlusPlus on github if you want to do more with this.)
	ld a, SPRITE_RED
	ld hl, wVarSprite1
	ld [hl], a
...
; Initialize map script numbers
	...
	ld bc, W_GAMEPROGRESSFLAGSEND - W_GAMEPROGRESSFLAGS
...
INCLUDE "engine/battle/ai_load_move_data.asm" ;Used to help trainer_ai now that it is in $2D
;INCLUDE "engine/battle/trainer_ai.asm" ;Moved to $2D
...
SECTION "Trainer AI", ROMX,BANK[$2D] ;Try to move trainer AI stuff, so we can add more.

INCLUDE "engine/battle/trainer_ai.asm"
INCLUDE "engine/battle/trainer_load_evs.asm"
INCLUDE "data/trainer_evs.asm"

SECTION "Fast Square Root", ROMX, BANK[$2E]
;Square Root
;Inputs:
...
SECTION "Extra Sprites", ROMX, BANK[$2F]	
