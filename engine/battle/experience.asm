GainExperience: ; 5524f (15:524f)
	...
	ld hl, wPartyMon1 ;pointing at Species.
	xor a
	ld [wWhichPokemon], a
.partyMonLoop ; loop over each mon and add gained exp
	...
	ld de, (wPartyMon1HPExp + 1) - (wPartyMon1HP + 1) ;offset from hp, to hpexp.
	add hl, de ;pointing at HPEXP
	ld d, h
	ld e, l
...
.nextBaseStat
	dec c
	jr z, .calculateExpGain
	inc de
	inc de
	jr .gainStatExpLoop
.calculateExpGain
	;once we are here, it is de + 7 to partyMonLevel.
	push de
	pop hl ; load de's address into hl.
	ld bc, 7
	add hl, bc
	ld a, [hl]
	ld b, a ; b now contains partyMonXLevel
	xor a
	ld [H_MULTIPLICAND], a
	...
	ld a, [wEnemyMonLevel]
	cp b
	jr c, .noLevelScale ; a < b ie player's level is higher
	sub b
	ld b, a
	;srl b ;half the difference between player and enemy's level
	ld a, [wEnemyMonLevel]
	add b
.noLevelScale
	ld [H_MULTIPLIER], a
...
.next2
	...
	ld [wd71f], a ;store current level for move loop later.
	ld a, [W_CURENEMYLVL]
	push af
	push hl
	ld a, d
...
.printGrewLevelText
	...
	;[wd71f] is pokemon's old level.
	;[W_CURENEMYLVL] is pokemon's new level.
	ld a, [wd71f]
	ld b, a
	ld a, [W_CURENEMYLVL]
	sub b
	ld c, a ;c now contains the number of levels gained.
	ld a, b
	ld [W_CURENEMYLVL], a
.loopLearnMoves
	push bc
...
.divideLoop
	...
	ld [H_DIVISOR], a
	cp $2
	ret c ; return if going to divide by 1.
	ld b, $2
