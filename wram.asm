...
wBattleAIWhichMon:: ;cd3a ;byte taken over for hack.	
wWastedByteCD3A:: ; cd3a
; written to but nothing ever reads it (used for sprite controlled walking.)
	ds 1
...
wd119:: ds 1 ; written to from W_CURMAPTILESET but never read. Used to debug map loading???
...
W_CURRENTBOXNUM:: ds 2 ; current box number
W_NUMOFHOFTEAMS:: ds 1 ; number of HOF teams
...
W_GAMEPROGRESSFLAGSEND::
	...
	ds 55 ;56 - number of var sprites.
...
; see they are all set to SPRITE_RED by default, currently.
wVarSprites::
wVarSprite1::
	ds 1
...
wd743:: 
	ds 1 ; used with surge gym trash cans
wSecondLockTrashCanIndex::
wd744::
	ds 1 ; also used with surge gym trash cans

	ds 2
	
wEventFlags::
	;ds 320 ;We eventually can get rid of all the stuff from here to wLinkEnemyTrainerName
			;once we replace all the event checks with the proper macros. 
